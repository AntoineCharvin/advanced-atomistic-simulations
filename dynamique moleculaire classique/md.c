...
// Fonction pour calculer le centre de masse
double* com(int ndim, int n, double pos[]) {
    int i, j;

    // Allouer dynamiquement le tableau com
    double *com = (double *)malloc(ndim * sizeof(double));

    // Vérifier si l'allocation de mémoire a réussi
    if (com == NULL) {
        fprintf(stderr, "Erreur d'allocation de mémoire pour le tableau com\n");
        exit(EXIT_FAILURE);
    }

    // Initialiser le tableau com
    for (i = 0; i < ndim; i++) {
        com[i] = 0.0;
    }

    // Calculer la somme des positions
    for (j = 0; j < n; j++) {
        for (i = 0; i < ndim; i++) {
            com[i] += (pos[j * ndim + i]) / n;
        }
...
void report(int what, FILE *fp, int ndim, int n, int steps, double pos[], double vel[], double epot, double dt) {
    double ekin;
    double *comass_a, *comass_b;

    if (what == 0) {
        fprintf(fp, "# %6s %14s %14s %14s %14s %14s %14s %14s %14s %14s\n", "Steps", "Epot", "Temp", "Ekin", "Etot", "center of mass Ax", "center of mass Ay","center of mass Bx", "center of mass By", "time");
    } else if (what == 1) {
        ekin = kinetic(ndim, n, vel);

        // Calcul du centre de masse pour la première moitié
        comass_a = com(ndim, n / 2, pos);

        // Calcul du centre de masse pour la deuxième moitié
        comass_b = com(ndim, n - n / 2, pos + n * ndim / 2);

        fprintf(fp, "%8d %14f %14f %14f %14f %14f %14f %14f %14f %14f\n", steps, epot, 2.0 * ekin / (n * ndim), ekin, epot + ekin, comass_a[0], comass_a[1], comass_b[0], comass_b[1], steps * dt);
...
void write_xyz (FILE *fp, int ndim, int n, double pos[], int nsteps) {
  int i, j;
  fprintf(fp, "%d\n", n);
  fprintf(fp, "%d\n", nsteps);
...
int main (int argc, char * argv[]) {
  int i, n, k, ndim, nsteps = 500;
  int stepcount = 0;
  double *box, dt = 0.02, rc = 2.5, epot;
  double *pos, *vel, *forc;
  FILE *file, *log, *output;

  file = fopen("nano_A_B.xyz","r");
  log  = fopen("output.log","w");
  output = fopen("data.xyz", "w");
  ...
  report(0,log,ndim,n,0,pos,vel,epot,dt);
  /* Main MD loop */
  for (i=0; i<nsteps; i++) {
    evolve(ndim,n,dt,rc,box,pos,vel,forc,&epot);
    if (i % 10 == 0) report(1,log,ndim,n,i,pos,vel,epot,dt);
    write_xyz(output, ndim, n, pos, stepcount);
    stepcount++;
  }
